...
import { ThumbsUp, ThumbsDown, CheckCircle, Loader, User, Briefcase, Star, MessageSquare, Clock, Users } from 'lucide-react';
...
export default function VotingPoll() {
  ...
  const fetchPolls = async () => {
    ...
    try {
      // Fetch all dispute created events
      ...
      // Fetch registry for profile data
      const registryObject = await suiClient.getObject({
        id: REGISTRY_ID,
        options: { showContent: true },
      });
      ...
      for (const event of disputeEvents.data) {
        ...
        try {
          ...
          if (pollObject.data?.content && 'fields' in pollObject.data.content) {
            ...
            // Determine user's vote if they voted
            let userVote: 'talent' | 'client' | null = null;
            if (hasVoted) {
              // Note: We can't directly determine which way they voted from on-chain data
              // This would require tracking vote events separately
              userVote = null;
            }
      ...
      // Sort: Active polls first, then by most recent
      ...
      showNotif('Failed to load polls');
    } finally {
      ...
      signAndExecute(
        { transaction: tx },
        {
          onError: (err) => {
            ...
            showNotif('Failed to submit vote. Try again');
            setVoting(false);
          },
          onSuccess: () => {
    ...
    return (
      <div className="w-full min-h-screen bg-[#1A031F] px-4 md:pl-10 py-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Please connect your wallet to view and vote on disputes</p>
        </div>
      </div>
    );
  }
    ...
    <div className="px-4 md:pl-10 py-6 text-white rounded-xl bg-[#1A031F]/80 min-h-screen relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <h1 className="text-white font-bold mt-2 md:mt-4 text-xl">Voting Polls</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#2B0A2F]/70 p-5 rounded-2xl border border-[#641374]/50 flex justify-between items-center">
          <div>
            <h3 className="text-gray-400 text-sm">Total Disputes</h3>
        ...
        <div className="bg-[#2B0A2F]/70 p-5 rounded-2xl border border-[#641374]/50 flex justify-between items-center">
          <div>
            <h3 className="text-gray-400 text-sm">Active Polls</h3>
      ...
      <div className="bg-[#2B0A2F]/70 p-5 rounded-2xl border border-[#641374]/50 shadow-md">
        ...
        {loadingPolls ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-sm">No disputes available yet.</p>
          </div>
        ) : (
                  ...
                  {poll.resolved ? (
                    ...
                    <div className="flex gap-3 w-full">
                      <button
                        ...
                        disabled={voting || poll.hasVoted || !canVote}
                    ...
                    <div className="w-full bg-gray-500/10 border border-gray-500/30 rounded-xl p-4 text-center">
                      <p className="text-gray-400 text-sm mb-2">Poll Ended</p>
                      <div className="flex items-center justify-center gap-6 mt-3">
                        <div className="text-center">
                          <p className="text-xs text-gray-400">Talent Votes</p>
                          <p className="text-lg font-bold text-purple-400">{poll.votesForTalent}</p>
